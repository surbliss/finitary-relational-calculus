...
nix run nixpkgs#omnix -- \
  init github:srid/boolean-database -o ./yourproject
```
...
Full instructions: https://srid.ca/boolean-database/start
...
- Run the application without installing: `nix run github:srid/boolean-database` (or `nix run .` from checkout)
- Common workflows
  - Adding library dependencies in Nix: https://community.flake.parts/haskell-flake/dependency
  - Adding tests: https://srid.ca/boolean-database/tests

## Discussions

Questions? Ideas? Suggestions? Join our [NixOS Asia community](https://nixos.asia/en/#community) or post in [Github Discussions](https://github.com/srid/boolean-database/discussions).
